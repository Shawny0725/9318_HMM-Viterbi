...
# Query_Label ='./toy_example/Query_Label'
...
def init_state_matrix(State_File):
    ...
    for i in range(len(State_matrix)):
        ...
        # State_matrix[i]=(data_in_line + 1) / (sum(data_in_line) + State_number - 1)
    # print(State_matrix.sum(axis=1))
    initial_transition_pro_list=State_matrix[State_number-2,:-2]
    # print('initial_transition_pro_list',initial_transition_pro_list)
...
def init_symbol_matrix(Symbol_File,State_File):
    ...
    # print('Symbol_matrix_Symbol_matrix',Symbol_matrix)
    # print(Symbol_matrix)
...
def init_symbol_matrix_for_q3(Symbol_File,State_File):
    ...
    Symbol_File_data_lines=Symbol_File_data.read().splitlines()
    Symbol_number=int(Symbol_File_data_lines[0])
    # print('Symbol_number',Symbol_number)
    for line_num in range(1,Symbol_number+1):
        symbol_list.append(Symbol_File_data_lines[line_num])
    ...
    # print(np.shape(nj))
    for i in range(len(Symbol_matrix)):
        # print('Symbol_matrix[i]',Symbol_matrix[i])
        data_in_line=Symbol_matrix[i]
        count=0
        ...
        for idx in range(Symbol_matrix.shape[1]):
            if Symbol_matrix[i][idx]==0:
                # print('Symbol_number+1-count',Symbol_number-count)
    ...
    for query in Query_list:
        ...
        for i in range(0,columns):
            ...
            for j in range(nb_of_state):
                ...
                # print('i=',i,'j=',j,'begin_state=', begin_state, 'transition_pro=', transition_pro, 'emission_pro=', emission_pro,'transition_pro*emission_pro',transition_pro*emission_pro,'value=',value)
                max_value=max(value)
                l.append(max_value)
                path=list(value).index(max_value)
                temp_list.append(path)
            b = []
            for temp_index in range(len(temp_list)):
                pre_path=temp_list[temp_index]
                a = [i for i in (path_list[pre_path])]
                a.append(temp_index)
                b.append(a)
            path_list = b
            begin_state =np.array(l)
            viterbi_matrix[i]=l

        end_value=begin_state * end_matrix
        max_path_index=list(end_value).index(max(end_value))
        viterbi_matrix[-1]=end_value
        highest_pro_path=path_list[max_path_index]
        highest_pro_path.append(nb_of_state+1)
        highest_pro_path.append(np.log(max(end_value)))
        result_list.append(highest_pro_path)
        # print(highest_pro_path)

    # print(viterbi_matrix.T)
    return result_list
...
def advanced_decoding(State_File, Symbol_File, Query_File):
    ...
    for query in Query_list:
        ...
        highest_pro_path = []
        for i in path_list:
            i.append(nb_of_state + 1)
            highest_pro_path.append(i)
        result_list.append(findTOP_K(highest_pro_path, list(np.log(end_value)), 1)[0])
    print(result_list)
    return result_list


def check_accuracy(State_File,Symbol_File,Query_File,Query_Label):
    # result =top_k_viterbi(State_File,Symbol_File,Query_File,1)
    # result =viterbi_algorithm(State_File,Symbol_File,Query_File)
    result =advanced_decoding(State_File,Symbol_File,Query_File)
    ...
    with open(Query_Label,'r') as file:
        ...
        for j in range(len(lines)):
            ...
            print(label)
            my_ans = np.array(result[j][:-1])
            print(my_ans)
            print(my_ans == label)
            wrong_labels += np.sum((label!=my_ans))
    print(wrong_labels)
    print(counter)
    print('acc', 1 - (wrong_labels / counter))

check_accuracy(State_File,Symbol_File,Query_File,Query_Label)
